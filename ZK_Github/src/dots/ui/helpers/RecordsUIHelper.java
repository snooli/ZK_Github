...
import dots.ui.comparators.DocumentComparator;
import dots.ui.comparators.RecordComparator;
import dots.ui.listeners.trees.DoubleClickDocumentEventListener;
...
public class RecordsUIHelper {
	...
	public static void populateRecords(String fileSystemId) {
		...
		for (RegistryEntryBean record : records) {
			...
			List<DocumentDescriptionBean> dds = dotsClient.get().getBaseService()
	...
	public static boolean isRecordSelected() {
		...
		if (selectedItem == null) {
			...
			RegistryEntryBean record = dotsClient.get().getSearchService()
